...
# Local imports
from .validation import crossref_api, property_units, yaml
        ...
        for val in dp:
            ...
            if property_id.get(val.tag) in datagroup_properties:
                ...
                if units in ["mole fraction", "mass fraction", "mole percent"]:
                    ...
                    # assume molar ppm, convert to mole fraction
                    warn("Assuming molar ppm in composition and converting to mole fraction")
                    spec["amount"] = [float(val.text) * 1.0e-6]
                    units = "mole fraction"
                elif units == "ppb":
                    # assume molar ppb, convert to mole fraction
                    warn("Assuming molar ppb in composition and converting to mole fraction")
                    spec["amount"] = [float(val.text) * 1.0e-9]
                    units = "mole fraction"
                else:
                    raise KeywordError(
                        "composition units need to be one of: mole fraction, "
...
def respth2ck(argv=None):
    ...
    properties = ReSpecTh_to_ChemKED(
        filename_xml, args.file_author, args.file_author_orcid, validate=True
    )
